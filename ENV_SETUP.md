...
## 10. 下一阶段环境切换说明（真实后端/真实LLM）

当前联调默认使用 Mock 服务（3200/3300）。进入下一阶段后，建议通过中台环境变量切换上游地址：

1. 保持前端调用中台不变（前端不直连真实后端/真实LLM）。
2. 在 `PROJECT/MIDDLEWARE/server/.env` 中配置真实上游地址（变量名以中台实现为准）。
3. 切换后先跑 `healthz` 与最小创建仿真流程，再进行全链路压测。
