...
function setupWebRtcPlayer(htmlElement, config) {
    ...
    webRtcPlayerObj.onDataChannelMessage = function(data) {
        ...
        if (view[0] === ToClientMessageType.QualityControlOwnership) {
            ...
            let setValueIfPresent = function(id, value) {
            ...
            let setCheckedIfPresent = function(id, checked) {
                let el = document.getElementById(id);
                if (el !== null) {
                    el.checked = checked;
                }
            ...
            if (settingsJSON.Encoder) {
                setValueIfPresent('encoder-rate-control', settingsJSON.Encoder.RateControl);
                setValueIfPresent('encoder-target-bitrate-text', settingsJSON.Encoder.TargetBitrate > 0 ? settingsJSON.Encoder.TargetBitrate / 1000 : settingsJSON.Encoder.TargetBitrate);
                setValueIfPresent('encoder-max-bitrate-text', settingsJSON.Encoder.MaxBitrate > 0 ? settingsJSON.Encoder.MaxBitrate / 1000 : settingsJSON.Encoder.MaxBitrate);
                setValueIfPresent('encoder-min-qp-text', settingsJSON.Encoder.MinQP);
                setValueIfPresent('encoder-max-qp-text', settingsJSON.Encoder.MaxQP);
                setCheckedIfPresent('encoder-filler-data-tgl', settingsJSON.Encoder.FillerData == 1);
                setValueIfPresent('encoder-multipass', settingsJSON.Encoder.MultiPass);
            }
            if (settingsJSON.WebRTC) {
                setValueIfPresent('webrtc-degradation-pref', settingsJSON.WebRTC.DegradationPref);
                setValueIfPresent("webrtc-max-fps-text", settingsJSON.WebRTC.MaxFPS);
                setValueIfPresent("webrtc-min-bitrate-text", settingsJSON.WebRTC.MinBitrate / 1000);
                setValueIfPresent("webrtc-max-bitrate-text", settingsJSON.WebRTC.MaxBitrate / 1000);
                setValueIfPresent("webrtc-low-qp-text", settingsJSON.WebRTC.LowQP);
                setValueIfPresent("webrtc-high-qp-text", settingsJSON.WebRTC.HighQP);
            }
        } else {
